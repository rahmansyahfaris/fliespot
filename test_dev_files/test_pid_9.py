...
# Test PID 9:
# - rotations (there is a little twist here, the orientation stays and doesn't change by yaw rotation)
# - not using any mathematical formulas for the orientation adjustment, this is still by switching the variables (which is still problematic)
# - also logs yaw, just see if it is steady and accurate when flying (because it is not when not flying, so perhaps flying can be different)
...
def log_pos_callback(timestamp, data, logconf):
    ...
    print(f"[{current_datetime.strftime('%Y-%m-%d %H:%M:%S.%f')}] ET: {elapsed_time * 1000:.3f} ms | x_cur: {round(pos_current_x,2)} | y_cur: {round(pos_current_y,2)} | x_log: {round(pos_logging_x,2)} | y_log: {round(pos_logging_y,2)}  | yaw_log: {round(yaw_logging,2)}")
    ...
    with MotionCommander(scf, default_height=DEFAULT_HEIGHT) as mc:
        ...
        while True:
            ...
            if initial_elapsed_time > initial_timeout:
                print("STATUS: Initial timeout, condition isn't met")
                break
        ...
        pos_desired_x = 0.0
        pos_desired_y = 0.0
        pid_x = PID(Kp_x, Ki_x, Kd_x, setpoint=pos_desired_y)
        pid_y = PID(Kp_y, Ki_y, Kd_y, setpoint=pos_desired_x)
        hold_duration = 3
        ...
        while True:
            ...
            if hold_time_elapsed > hold_duration:
                break
            control_velocity_x = pid_x.compute(pos_current_y)
            control_velocity_y = pid_y.compute(pos_current_x)
            mc.start_linear_motion(control_velocity_x, -control_velocity_y, 0)
            time.sleep(0.1) # little delay to not overwhelm the drone
        mc.start_linear_motion(0, 0, 0) # stay still
